...
class Config(object):
    """Configuration Object, member fields must be overwritten using
        :meth:`create_config` after import.
    """
    def __init__(self):
    ...
    def create_config(self, number_of_trees, data_size_for_tree):
        ...
        Args:
            ...
            data_size_for_tree (int): Determines number of rows of training
                data that go into every tree construction.
        """
...
class Forest(object):
    ...
    def bootstrap(self, data):
        ...
        Record are stored in list of :class:`np.ndarrays` form which trees are
        created. Record that were not selected can by found in
        :attr:`self.out_of_bag`.
        ...
        for _ in range(config.number_of_trees):
            training_data = data.sample(config.data_size_for_tree,
                                        replace=True)
            self.out_of_bag = self.out_of_bag[
                ~self.out_of_bag.index.isin(training_data.index)]
